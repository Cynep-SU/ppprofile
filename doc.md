...
Снова мы можем увидеть, что самым узким местом является ввод.
...
В итоге мы получаем 11%, что довольно интересно, потому что при одинаковом количестве вызове функций random при обоих случаях тратиться разное количество времени. Также добавляет времени метод списка .append(<value>)
